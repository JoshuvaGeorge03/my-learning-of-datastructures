...
const { dummyPredicate, dummyArr, doNothingExceptReturningPassedArgument } =
  utils;
const {
  convertToFalseOnlyIfValueIsNullOrUndefined,
...
export class LinkedList {
    ...
    if (rawIndex === 0) {
      ...
      if (currentNode) {
        ...
        if (this.toArray().length - 1 >= rawIndex) {
          ...
          throw new Error(
            `index ${index} to be added is larger than list length of ${
              this.toArray().length
            }`
          );
        }
  ...
  searchBasedOnValue(data) {
    ...
    if(currentNode) {
        return { node: currentNode, index: index, data: currentNode.data };
    }
    return null
  }
  ...
  find(predicateCb = dummyPredicate) {
    ...
    if(currentNode) {
       return { node: currentNode, index: currentIndex, data: currentNode.data };
    }
    return null;
  }
